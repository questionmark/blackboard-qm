	...
	public static PerceptionContext getInstance( Context ctx ) throws PerceptionDataException {
		if( ctx != null && ctx.getUser() != null && ctx.getCourse() != null && ctx.getUser().getSystemRole() != User.SystemRole.GUEST){
			...
			if( settings.getIsSynchronizationEnabled() ){
				...
				if( pContext.group == null ){
					pContext.group = createGroup( course, course_id );
				...
				if( settings.getCreateAuthoringRights() && isCourseAdmin ){
					...
					// Authoring rights
					// create topic
					PerceptionTopic topic = null;
					String topicAndFolderName = removeInvalidFolderCharacters(course_id);
		
					if( BbUtils.isNullOrEmpty(courseSettings.getTopicID()) ){ // we assume parent_id is also null
						try{
							topic = createTopic( course, removeInvalidFolderCharacters(course_id) );
						...
						// check exists
						try{
							topic = PerceptionServiceManager.getPerceptionTopicLoader().getTopicById( courseSettings.getTopicID() );
						}catch( PerceptionDataException pde ){
							// log.logDebug( "failed to load topic" );
							// look for topic of same name parent
							if( !BbUtils.isNullOrEmpty(courseSettings.getTopicParentID()) ){
								try{
									topic = PerceptionServiceManager.getPerceptionTopicLoader().getTopicByNameAndParentId( topicAndFolderName, courseSettings.getTopicParentID() );
								} catch( PerceptionDataException pde2 ){
									//log.logDebug( "topic not found by name and parent id" );
								}
							}
							
							if( topic == null) {
								try{
									topic = createTopic( course, topicAndFolderName );
								} catch( PerceptionDataException pde2 ){
									log.logDebug( "failed to create topic, yet is doesn't exist!" );
								}
							}
						}
					}
					...
					if( topic != null ){
						...
						try{
							...
							courseSettings.setTopicParentID( topic.getParentId() );
					...
					PerceptionAssessmentFolder folder = null;
					if( BbUtils.isNullOrEmpty(courseSettings.getFolderID()) ){
						try{
							folder = createFolder( ctx.getCourse(), topicAndFolderName );
						} catch( PerceptionDataException pde ){
						...
						//check exists
						try{
							folder = PerceptionServiceManager.getPerceptionAssessmentFolderLoader().getAssessmentFolderById( courseSettings.getFolderID() );
						}catch( PerceptionDataException pde ){
							
							if( !BbUtils.isNullOrEmpty(courseSettings.getFolderParentID()) ){
								try{
									folder = PerceptionServiceManager.getPerceptionAssessmentFolderLoader().getAssessmentFolderByNameAndParentId( topicAndFolderName, courseSettings.getFolderParentID() );
								}catch( PerceptionDataException pde2 ){
									//log.logDebug( "folder not found by name and parent id" );
								}
							}
							if( folder == null ){
								try{
									...
									log.logDebug( "failed to create folder, yet is does not exist!" );
								}
							}
						}
					}
					...
					if( folder != null ){
						...
						try{
							...
							courseSettings.setFolderParentID( folder.getParentId() );
