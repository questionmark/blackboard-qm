...
public class FullSynchronisationServlet extends SynchronisationServlet {
	...
	public boolean doContent(HttpServletRequest request, HttpServletResponse response, Context ctx) throws ServletException, IOException{
		...
		try{
			...
			// added for 4.4
			// removing unavailable accounts
			courseUsers = courseUsers.getFilteredSubList( new CourseMembershipAvailabilityFilter() );
			...
			if( groupParticipantsToRemove != null && groupParticipantsToRemove.size() > 0 ){
				...
				removeParticipantsNoLongerInBbCourse( groupParticipantsToRemove, group, stepCount, clientWriter, locale, log );
	...
	protected static List filterGroupParticipantsToRemove( List groupParticipants, BbList courseStudents, boolean useExternal ){
	...
	private static List getGroupMembershipsToRemove( List groupParticipantsToRemove, String group_id, boolean isAdmin ){
	...
	protected static int removeParticipantsNoLongerInBbCourse( List groupParticipantsToRemove, PerceptionGroup group, int stepCount, PrintWriter clientWriter, Locale locale, PerceptionLog log ) throws IOException, PerceptionDataException {
