...
<bbNG:learningSystemPage ctxId="ctx" title="Questionmark Perception connector">
		...
		<bbNG:dataCollection>
			<bbNG:stepGroup active="true" title="Course Settings">
				<bbNG:step title="Enter Information" hideNumber="true">
					...
					<input type="checkbox" id="hide_schedules" name="hide_schedules" value=<%= panel.courseSettings.getProperty("hide_schedules","0").equals("1")%> />
					<br/>
			...
			<bbNG:step title="Enter Information">
				...
				<bbNG:dataElement label="Assessment name">
					<select name="assessment">
						<% 
						String last_ID = "";
						for(int i = 0; i < panel.assessmentList.length; i++) { 
							String next_ID = panel.assessmentList[i].getAssessment_ID();
							if (!next_ID.equals(last_ID)){%>
						<option value="<%=next_ID%>"><%=panel.assessmentList[i].getSession_Name()%></option>				
						<%		last_ID = next_ID;
							}					
						} %>
					</select>
	...
	<bbNG:inventoryList collection="<%=panel.scheduleInfo %>" objectVar="s"
		...
		<bbNG:listElement name="launch" label="Launch">
			<% if (s.launchURL!=null) {
				%><a href="<%=s.launchURL%>">Start the Test</a><%
			} else {
				%>Unavailable<%
			}
			%>
